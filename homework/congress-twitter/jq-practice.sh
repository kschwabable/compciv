...
echo '6. '
cat data-hold/legislators-current.json | jq '.[] .bio .religion' | sort -r | uniq -c | sort -r | tr -d '"'

echo '7. '
cat data-hold/legislators-current.json | jq --raw-output '.[] .terms[0] .start' | sort | cut -d '-' -f 1 | grep -c '2015'

echo '8. '
cat data-hold/legislators-current.json | jq --raw-output '.[] .terms[] .start' | sort | uniq | head -n 1 | cut -c 1-4
...
echo '15. '
cat data-hold/legislators-social-media.json |   jq --raw-output '.[]  | [.id .bioguide, .social .twitter] | @csv'   | grep -v ',"' | head -n 10
